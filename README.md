...
> **⚠️ This repo has been superseded by [now-showing-ha](https://github.com/rusty4444/now-showing-ha)** — a unified HA App that supports Plex, Kodi, Jellyfin, and Emby in one package, with new themed accents, lots of upgrades, no code setup, and a full visual editor. This repo will remain available but will not receive new features.

---

A full-screen cinema marquee display for Home Assistant that shows what's currently playing on Emby. Designed for wall-mounted tablets using Fully Kiosk Browser.
...
- Filters to a specific Emby `media_player` entity so only your playback shows up
...
- **Home Assistant** with the [Emby integration](https://www.home-assistant.io/integrations/emby/) configured
- **Emby** server running and accessible from your Home Assistant instance
...
| `automations/emby_now_showing_display.yaml` | Automation to auto-switch a tablet when playback starts/stops |
...
const EMBY_ENTITY = 'media_player.emby';            // Your Emby media_player entity ID
```
...
Navigate to `http://YOUR_HA_IP:8123/local/now_showing.html` in a browser to test it. If Emby is playing something, you should see the poster appear.
...
1. Copy the contents of `automations/emby_now_showing_display.yaml` into your `automations.yaml` file, or recreate it through the HA UI
2. Update the following values in the automation:

| What to change | Where in the file | How to find yours |
|---|---|---|
| **Emby entity** | `entity_id: media_player.emby` (appears in triggers and conditions) | Go to **Developer Tools → States**, search for your Emby player. Look for a `media_player.*` entity whose state changes to `playing` when Emby plays something. |
...
- Polls Home Assistant's API every 5 seconds for the Emby `media_player` entity state
...
- The automation triggers when the Emby `media_player` state changes to `playing`, `idle`, or `off` — it waits 5 seconds then loads the Now Showing page, or waits 10 seconds after playback stops to return to your dashboard
...
| Emby entity | `EMBY_ENTITY` in script | `media_player.emby` |
...
- **Blank poster**: Check that your HA token is valid and the Emby `entity_picture` URLs are accessible from the device
- **Nothing showing while Emby is playing**: Verify your `EMBY_ENTITY` matches the entity ID in Developer Tools → States
- **Automation not triggering**: Confirm your Emby `media_player` entity state changes correctly when playback starts/stops. Check **Developer Tools → States** to monitor state changes.
